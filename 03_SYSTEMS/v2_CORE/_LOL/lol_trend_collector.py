...
class LolTrendCollector:
    ...
    def collect_champ_trends(self, champion: str, role: str) -> dict:
        ...
        【最新パッチに統計データが無い場合の注意】
        パッチがリリースされた直後などで、最新パッチの勝率・ピック率・バン率・ティアがまだ
        検索結果に十分出てこない場合、それらしい数値を推測・創作しないこと。その場合は
        直近で実際にデータが存在する1つ前のパッチの統計を代わりに採用し、"patch"フィールドには
        実際にデータの根拠とした方のパッチ番号を正直に記載すること（最新パッチ番号を
        偽って記載しないこと）。
        """
