...
【厳格な指示・ハルシネーション対策】これら3項目は、lolalytics/u.gg/mobalytics等のビルドタイミングデータや、実際のジャングルクリア解説記事・動画で明示的に言及されている、このチャンピオン固有の数値のみを報告してください。他のチャンピオンにも当てはまりそうな「平均的・一般的な目安」を当てはめて埋めることは絶対禁止です（例えば多くのチャンピオンに機械的に同じような秒数を割り当てるのは典型的なハルシネーションです）。google_search grounding で実際にこのチャンピオンの情報が見つからない項目は、無理に数字を作らず必ずnullにしてください。3項目とも見つからなければ、jg_style全体からこれらのキーを省略しても構いません。
