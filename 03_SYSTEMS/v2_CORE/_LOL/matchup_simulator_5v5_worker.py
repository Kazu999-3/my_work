...
def main():
    ...
    # 10名のチャンピオンの日本語公式名のマッピングを作成
    name_mapping = {}
    for role, name in blue_champs.items():
        name_mapping[name] = CHAMPION_JA_MAP.get(name.lower(), name)
    for role, name in red_champs.items():
        name_mapping[name] = CHAMPION_JA_MAP.get(name.lower(), name)

    mapping_rules = "\n".join([f"- {en} は日本語解説内では必ず「{ja}」と表記してください。" for en, ja in name_mapping.items()])
...
【重要：日本語翻訳・呼称ルール】
解説文を生成する際は、以下のチャンピオン呼称（公式日本語名）を【厳密に】使用してください。英語表記のまま出力したり、誤った音訳（例：Aatrox を アートロックス と呼ぶなど）をすることは絶対に避けてください。
{mapping_rules}
