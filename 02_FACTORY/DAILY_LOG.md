...
### 🎯 本日の主タスク
- **定期募集最多ランク帯集計ルール改定 ＆ バランサーMMR表示強化**
  - **定期募集判定ルールの改定**:
  ...
  - **テスト・品質検証**:
    - `npm test`（36テスト全件通過）および TypeScript型チェック（`tsc --noEmit` エラー0件）を検証完了。

### 💡 再利用可能な3行ナレッジ (Coupen/つくラボAI原則)
1. **ランク集約ルールの正規化**: 端点（高レート/未ランク）の人数がばらつくコミュニティ戦では、最多帯判定時に「エメラルド以上➔プラチナ」「アイアン・未ランク➔ブロンズ」のように中核帯へ集約マッピングすることで、成立可能な部屋の基準を安定して算出できる。
2. **手動入れ替えUIの情報量**: バランサーでの選手スワップ時、セレクトボックス内にMMRを即時インライン表示することで、画面スクロールやカルテ確認の手間をゼロにし、対面MMR差の微調整ミスを未然に防げる。
3. **Botとポータルガイドの完全同期**: Botの判定コードを変更した際は、ユーザーの混乱を防ぐため即座にポータルの公式ガイド（`GuideRulesTab`）にも同様のルールを同時反映しておく。
