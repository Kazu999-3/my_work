...
### 🎨 攻略ナレッジハブ・チャンピオン攻略辞典のモダンライトUI刷新 ＆ 導線整理
...
2. `04_PORTAL/src/app/champions/page.tsx`:
   - `/champions` を純粋な「👑 チャンピオン攻略辞典」へ一本化。
   - 画面上部を圧迫していた巨大な5タブバー（戦術取り込み、ヘルス等）を撤去し、コンパクトなヘッダーへ刷新。
   - 一般ユーザー向けの「📖 レーン・マクロ攻略」「📒 攻略記事一覧」および管理者専用の「⚙️ ナレッジ管理」を右上のスマートなサブナビへ再配置。
3. `04_PORTAL/src/app/champions/components/ChampionVisualDashboard.tsx`:
   ...
   - 罠ビルド表示やJG実測数値バッジもコントラストと可読性を最適化。
4. `04_PORTAL/src/app/champions/tabs/DictionaryTab.tsx`:
   ...
   - VS対面比較モードのカードも黒ベタ塗りからモダン白カードへ刷新し、自チャンプと相手チャンプの視認性を劇的に向上。
...
2. **管理機能とエンドユーザー閲覧画面の境界分離**: 1つの画面に管理機能（取り込み、データ修復、ヘルス監視）と同居させると、一般ユーザーの利用体験を損ない画面が散らかる。エンドユーザーが日常的に見る辞典機能に特化させ、管理機能は管理者専用ルート（`/admin/...`）へさりげなく導線を逃がすことがUI設計の基本である。
3. **白・スレート基調とゴールドアクセントの調和**: 清潔感のある白背景（`#f8fafc`）に対して、LoLのブランドアイデンティティであるゴールド/アンバー（`#c89b3c`, `amber-500`）をアクセントカラー（ボーダー、バッジ、微小なハイライトリング）として配することで、Hextechの世界観を保ちながら抜群の読みやすさとモダンさを両立できる。
