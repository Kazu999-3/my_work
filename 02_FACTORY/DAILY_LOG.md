...
2. `04_PORTAL/src/app/library/page.tsx` (新規作成) ＆ `04_PORTAL/src/components/Sidebar.tsx`:
   - **攻略ライブラリ（`/library`）の独立メニュー化**: チャンピオン辞典から切り離し、左サイドバーの一般メニュー（「ガイド ＆ 攻略」セクション）に `📒 攻略ライブラリ` を新設。一般メンバーもいつでも動画解析記事や戦術ノートを直感的に探せるように最適化。
...
3. `04_PORTAL/src/app/champions/page.tsx`:
   ...
   - 右側には「📖 レーン攻略」「📒 攻略ライブラリ」「📥 戦術取込（管理者のみ）」のワンクリック遷移リンクを常設。
