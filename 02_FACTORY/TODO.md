...
- [x] ~~**migration 78 `crash_used_tokens` の置き去り**~~ → 2026-09-22 対応済み。ファイルは残したまま中身を撤去し、
  「廃止・適用不要」の経緯（79番の`crash_sessions`方式へ置き換わった／参照0件／未適用）をコメントで記録した。
  削除ではなく無効化にしたのは、ファイル名が`_migrations`のキーで番号の連続性が壊れるため。
