...
## 🚨 ハードコード偽装データの一掃（2026-09-22 全量調査で36件検出 / 5件対応済み・31件残）

> **経緯**: コーチページの「実戦の罠（没理由DB）」がDB由来を名乗りながら全対面共通の
> ハードコード固定文だった件を起点に、ポータルAPI・UI/lib・Python(HUD/CLI)の3領域を全量調査。
> **同一構造の問題が36件**見つかった。共通する型は「**実データ取得に失敗したとき、それらしい内容で
> 埋め、失敗をユーザーに伝えない。かつ見出しにDB/実戦データ/客観/公式/確定と由来を示唆する語が付く**」。
> `known-regression-patterns` パターン5（ハルシネーション）の実装版と言える。

**対応済み（2026-09-22）**: ①個人成績の固定値がAIプロンプトを汚染 ②試合の教訓が保存されないスタブ
③未登録チャンピオンへの架空マッチアップ ④取得失敗を「適合済み」と伝える逆方向の嘘 ⑤出典不明の合否判定。

### 残タスク（深刻度順）

- [ ] **🔴 Python側HUD: 今日ポータルで直した問題がそのまま残っている**
  - `03_SYSTEMS/v2_CORE/_LOL/overlay/matchup_blueprint_engine.py` は docstring で「確定戦術バイブルから自動抽出」と明言しながら**ファイルもDBも読まず**、6体分のハードコードのみ。残り162体は全対面共通の汎用文を返し、HUDの「🚫 実戦の罠・NG行動警告（没理由連動）」として表示される。ポータル側で修正した構造と完全に同一。
  - `kill_line_calculator.py` は docstring が「168体すべてのDataDragon公式確定計算」と標榜するが、実体は**41体分の手書き**。残り127体は無言で `base_lvl6: 460.0` の固定値を使い、HUDに「HP 42%（517以下）で**即死確定**」と断言表示される。
  - `hud_state_engine.py` は `KillLineCalculator` をimportしながら一度も呼ばず、`kill_line` キーに別物（スキル名＋定型文）を入れている。
- [ ] **🔴 `generate_tactics_bible.py` の既定モードが架空データを「検証済み・公式」ラベルで永続化**
  - 17体分の手書き戦術文（「Lv6時: フルコンボで**約850 DMG**」等の根拠なき具体数値を含む）を、frontmatter `status: verified` / `source_type: official` 付きで `01_INTEL/tactics/*.md` へ書き込む。**ファイルに嘘の検証済みラベルが残る**ため実害が大きい。
  - ※2026-09-21に追加した `--from-db` モードは正直な実装（未取得項目は「未取得です」と明示）。**既定モードだけが旧来のまま**なので、そちらへ寄せるか既定モードを廃止する。
- [ ] **🔴 `analyzer/deep-intel`: 勝敗フラグだけから生成した偽スコア群**
  - `postgame-deep-analytics/route.ts`: `isWin ? 94 : 82` でビルド監査Sランク、`isWin ? '92%' : '80%'` でテンポ維持率、時間帯別勝率「序盤68%/中盤72%/終盤65%」など、**集計を一切せず勝敗の三項演算子で出す数値**が多数。1stアイテムは常に無条件で「最適解 👑」。Riot API由来の本物の数値と同一レスポンスに混在する。
  - 同ルート: PUUID未指定時に環境変数の「かずき」へ暗黙フォールバック。**他人が開くと他人の試合が自分の解析結果として表示される**。
- [ ] **🟡 AI失敗を隠す無言フォールバック（`coach/analyze`）**
  - Gemini のパース失敗時に汎用文を成功レスポンスとして返す箇所が3つ。特に `objective_priority` は **`confidenceScore: 80` という捏造した確信度**を付け「AI診断 確信度80%」と表示する。同ファイルの `counter_pick` は正しく502を返しており、そちらへ揃える。
- [ ] **🟡 `ChampionVisualDashboard.tsx`: 辞典画面の捏造フォールバック群**
  - `counterChampions` 欠損時にアーキタイプ別の**架空の「有利な相手/天敵」チャンピオン名**を生成（同アーキタイプなら全員同じ天敵になる）。「対面Sentinel実戦攻略メモ」の見出しで汎用文を出す箇所、`trend_items` 欠損時に具体的アイテム名を丸ごと補完する箇所、「実戦検証済み」バッジ付きで全チャンピオン共通の罠ビルドを出す箇所も同様。
- [ ] **🟡 その他（詳細は調査時のログ参照）**
  - `admin/live-match`: 3体のみの推奨カウンター、5体決め打ちの初動予測（`Prediction` というフィールド名で提示）
  - `lol/match-fights`: 交戦レビュー文が12通りの固定文。Shyvanaのみ専用フレーバー文があり特別に詳しいDBがあるかのように見える
  - `sessionAnalyticsCalculator.ts`: 不足ピース診断の推奨チャンピオンが固定リストで、誰を分析しても「KP@15が苦手」と決め打ちする理由文が付く
  - `EarlyJunglePathingCard.tsx`: 「初動3分ルート分岐（{自分} vs {相手}）」が3パターンの固定文
  - `gank_opportunity_engine.py`: 手書きレーティング表から「🟢 確実キル (85% UP)」という断定的な確率ラベルを生成
  - `extract_video_tactics.py` L430-459: 2026-09-20の修正漏れ。実タイムスタンプに固定テンプレ文を貼る経路が残存（下の分岐は正直化済み）
  - `comeback_compass_engine.py` / `dynamic_build_advisor.py` / `hud_state_engine.py` の各フォールバック
- [ ] **🟢 前提整備: DDragonにダメージデータが無い**
  - 調査の結論として「ハードコードをやめてDDragon実データへ寄せる」が最善だが、**現状の `ddragon_master_dict.json`（579KB / 865スキル）にはスキル名・CD・コストしか入っておらず、ダメージ数値とスケーリング係数が無い**（2026-09-22実測確認）。即死計算の根治には、まず `ddragon_master_sync.py` を拡張して DDragon の `effect`/`vars` を取り込む必要がある。
  - これが済めば、Python側 `kill_line_calculator.py` とポータル側 `BURST_PROFILES`（現在は**二重管理された別々の手書きテーブル**）を単一のソースへ統合できる。

### 修正の型（確立済み・横展開する）

`matchup-blueprint/route.ts` の `buildRejectedIntel()` と本日の各修正で確立した方針:
**①実データのみ返す ②無ければ null/空を返す ③UIは「まだ登録されていません」と正直に表示する
④静的なものは「一般論」「手入力値」と明示し、由来を偽る見出し（DB・公式・確定・客観・連動）を付けない。**
