...
- [ ] **旧形式カードの移行キック**: Actions → 「KTM Bot Cron Backup」→ Run workflow → mode に `weekly_recruit_migrate` を手入力して実行。今出ている旧カードの参加者3名を引き継いだ土日2枚の新カードを投稿し、旧カードは受付終了にする。冪等なので2回目は何もしない。実行後、旧カードのボタンが灰色になっているか・土曜カードに3名引き継がれているか・日曜カードが 0/10名 で出ているかを確認すること。
...
- [x] **デプロイ後の実機確認**（2026-09-23 完了）: タブ表示 / 開始→開封→引き返しでのコイン増減 / ページを閉じてからのラウンド復帰 / 敗北時の盤面公開、の4点を実際に遊んで確認済み。
- [ ] **大勝ち時のDiscord通知だけ未確認**（動かないと判明したわけではない）
  - 通知が飛ぶ条件は「20倍以上 または 払い戻し1万コイン以上」。到達確率はキノコ10個で6マス開けて約2.8%、キノコ5個で9マス開けて約4.2%しかないため、**数ラウンド遊んだ程度では出なくて当然**。
  - 送信経路は `sendShopNotification`（ショップ購入通知・スロットの15倍通知と同一の関数）をそのまま使っており、Mines側で変えたのは発火条件だけ。
  - ⚠️ 送信は `.catch(() => {})` で握りつぶしており、**失敗してもサーバーログにしか残らず事後に追えない**（スロット・ショップも同じ作り）。次にこの手の切り分けが必要になったら、`mines_sessions.settle_note` に通知の成否を残すようにすると追えるようになる。
...
- [ ] **`recruitments` に `(mode, start_at)` のユニーク制約が無い**（2026-09-23 発見・TOCTOU）
  - 定期カスタムの二重投稿防止は「SELECTしてから INSERT」のcheck-then-actで、DB側の保証が無い。現状はキック間隔が数時間空いているため実害は出ていないが、既知の再発パターン#3そのもの。
