...
- [ ] **第1弾 (MVP): 主要JGチャンピオン限定再解析 (約20〜30本)**
  ...
  - 目的: 最重要チャンプの実演クリップを最速で実戦投入可能な状態へ引き上げる。
- [ ] **第2弾: エラー・保留キュー（52件）の自動救済・再解析**
  - 対象: `youtube_queue` で過去に `error_generation` や `failed` となった動画。
  - 方式: 今回配備した多重モデルフォールバック（2.5-flash→1.5-flash→2.0-flash）およびVTT保護機構で復旧・再生成。
  - 目的: 取りこぼし資産の回収とキューの健全化。
- [ ] **第3弾: 全体（1058本）の定期ローテーション再解析（GitHub Actions / バッチ）**
  - 対象: 完了済み全1058本の動画。
  - 方式: API上限・レート制限を回避する段階的バッチスクリプトを作成し、1回10〜20本ずつ安全に定期ローテーション実行。
  - 目的: Sovereign OS全体の全ナレッジを最新のMacro/Why/How/Rejected基準へ完全統一。
