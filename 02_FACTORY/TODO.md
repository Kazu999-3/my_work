...
- [ ] **Step 1: 【プレイ前ナレッジ ＆ 対面計算基盤】**
  - [ ] **案A: 即死キルライン境界メーター** (`03_SYSTEMS/v2_CORE/_LOL/overlay/kill_line_calculator.py` / DataDragon公式確定ダメージ計算・イグナイト合算・AR/MR軽減適用)
  - [ ] **案B: 3段階勝ちパターン手順書** (Supabase実戦対面メモを `Lv1~2序盤立ち位置` ➔ `Lv3~5フリーズ基準` ➔ `Lv6~オールイン` に構造化)
  - [ ] ポータル `/coach` (試合前タブ) および `/champions` へのUI統合
- [ ] **Step 2: 【プレイ中オーバーレイ連動】**
  - [ ] Sovereign HUD (TAB画面・左側対面カード) への即死ライン警告＆現在フェーズ手順のリアルタイム描画
  - [ ] 劣勢時（-3000G）の「案C: 逆転戦術コンパス」HUD通知
- [ ] **Step 3: 【プレイ後ディープアナリティクス】**
  - [ ] **案1: 序盤15分メトリクス精密アナリティクス** (CSペース推移グラフ・トレード効率比率)
...
- [ ] **Step 4: 【完全勝利サイクルの結合 ＆ ナレッジ自動更新ループ】**
  - [ ] 試合終了時の勝敗・ファイトデータ・改善ポイントをチャンピオン辞典に自動反映し、次回のプレイ前ナレッジへ循環するループ検証
