...
- [ ] **⚠️ migration `82_mines_sessions.sql` の本番適用（ユーザー作業待ち）**
  - この環境に `DATABASE_URL` が無くスクリプトを流せなかったため未適用。**適用するまでゲームは動かない**（テーブルが無く、開始時に500が返る）。
  - 手順はどちらでもよい:
    1. `$env:DATABASE_URL="<接続文字列>"; node scripts/migrate.mjs`
    2. Supabase ダッシュボードの SQL Editor に `04_PORTAL/supabase/migrations/82_mines_sessions.sql` の中身を貼って実行
  - 内容はテーブル1つとインデックス3つの新規作成のみ。既存テーブルへの ALTER・DROP は無い。
  - 適用後、実際に1ラウンド遊んで「開始 → 数マス開ける → 引き返す」でコインが増減するかを確認すること。
