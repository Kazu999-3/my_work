---
name: patch-watcher
description: 公式のLeague of Legendsパッチノートサイトを定期的に監視し、新パッチリリースをトリガーとしてStrategistエージェントを自動起動するプラグイン。
---
...
3. **新規パッチを検知した場合**:
   - `current_patch` を更新。
   - SlackまたはDiscordへアラート送信：「新しいパッチ {X} がリリースされました。戦術分析を開始します。」
   - Strategistエージェントにパッチ番号を渡し、`/lol-tactics-production` ワークフローを自動起動させる。
