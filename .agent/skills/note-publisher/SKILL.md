...
- **下書き（Draft）として保存する（推奨・確認用）**
  ```bash
  .venv\Scripts\python.exe 03_SYSTEMS/v2_CORE/_MONETIZE/publisher.py note --title "【パッチ14.X】チャレンジャー直伝ジャングル解説" --body-file "02_FACTORY/note_drafts/target_article.md"
  ```
- **直接有料公開する（価格500円）**
  ```bash
  .venv\Scripts\python.exe 03_SYSTEMS/v2_CORE/_MONETIZE/publisher.py note --title "【パッチ14.X】チャレンジャー直伝ジャングル解説" --body-file "02_FACTORY/note_drafts/target_article.md" --publish --price 500
  ```
- **ログイン確認・初回ログイン実行時（ブラウザ画面を表示）**
  ```bash
  .venv\Scripts\python.exe 03_SYSTEMS/v2_CORE/_MONETIZE/publisher.py note --title "テスト" --body "テスト本文" --no-headless
  ```
...
- **JSONファイルからロードしてスレッド投稿（推奨）**
  ```bash
  .venv\Scripts\python.exe 03_SYSTEMS/v2_CORE/_MONETIZE/publisher.py x --tweets-json "02_FACTORY/note_drafts/tweets.json"
  ```
- **ログイン確認・初回ログイン実行時（ブラウザ画面を表示）**
  ```bash
  .venv\Scripts\python.exe 03_SYSTEMS/v2_CORE/_MONETIZE/publisher.py x --tweets "テスト投稿1" "テスト投稿2" --no-headless
  ```
...
2. **ログイン切れ（401 / リダイレクト）の対処**:
   ...
   - **手動ログインを促す際の手順**:
     - `python 03_SYSTEMS/v2_CORE/_MONETIZE/publisher.py note --title "ログイン用テスト" --body "ログイン" --no-headless` を実行。
     - ブラウザが開いたらユーザーに手動でログイン手続きを完了してもらい、エディタ画面（note）またはホーム画面（X）が表示されるのを確認後、コンソールで待機が解除されるのを待つ。

3. **投稿履歴の確認**:
   - 投稿完了時、コンソールに `SUCCESS:URL` が出力され、Supabase DB の `published_posts` テーブルにも履歴（プラットフォーム名、記事タイトル、URL）が自动的に保存されます。
