...
const docsSrcDir = path.join(__dirname, 'public/design_docs');
...
try {
  if (fs.existsSync(docsSrcDir)) {
    ...
    console.log('✅ [Sync] systemDesignMarkdown.ts has been successfully regenerated.');
  } else {
    console.log('⚠️ [Sync] docs directory not found. Skipping.');
  }
} catch (err) {
  console.error('❌ [Sync] Failed to synchronize:', err.message);
}
