        ...
        <div className="space-y-6">
          {fetchError ? (
            ...
            matches.map(match => {
              ...
              return (
                <div key={match.id} className="bg-stone-900 border border-stone-800 rounded-xl overflow-hidden shadow-lg">
                  <div className="bg-stone-800/50 px-4 py-3 md:px-6 flex flex-col md:flex-row justify-between items-start md:items-center border-b border-stone-800 gap-2 md:gap-0 flex-wrap">
                    <div className="flex items-center gap-2 md:gap-3 flex-wrap">
                      ...
                      {match.prediction && (
                        <div className="flex items-center gap-1.5 md:gap-2 text-xs bg-stone-950/60 px-2 py-0.5 md:py-1 rounded border border-stone-800 text-stone-300 ml-0 md:ml-2">
                          <span>予測勝率:</span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2">
                    ...
                    {/* RED TEAM */}
                    <div className={`p-4 border-t md:border-t-0 md:border-l border-stone-800 ${match.winning_team === 'RED' ? 'bg-red-950/20' : ''}`}>
...
function PlayerRow({ p }: { p: any }) {
  return (
    <div className="flex items-center justify-between bg-stone-950/50 p-2 rounded-lg border border-stone-800/50 hover:bg-stone-800/80 transition">
      <div className="flex items-center gap-2 md:gap-3 flex-1 min-w-0">
        ...
        {p.champion_name ? (
          <Image
            ...
            width={32}
            height={32}
            className="w-6 h-6 md:w-8 md:h-8 rounded-full border border-stone-700 shadow-sm flex-shrink-0"
            onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }}
          />
        ) : (
          <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-stone-800 border border-stone-700 flex items-center justify-center text-[10px] md:text-xs text-stone-500 flex-shrink-0">?</div>
        )}
        <span className="font-bold text-stone-200 text-xs md:text-sm truncate mr-2">{p.player_name}</span>
        <span className="text-[10px] font-mono bg-stone-900 text-stone-400 px-1.5 py-0.5 rounded border border-stone-800/80 mr-2 flex-shrink-0">
          {p.player_mmr ? `MMR: ${p.player_mmr}` : 'MMR: -'}
        </span>
      </div>
      <div className="flex items-center flex-shrink-0">
        <span className="font-mono text-xs md:text-sm tracking-tighter bg-stone-900 px-1.5 md:px-2 py-0.5 md:py-1 rounded">
          ...
          <span className="text-stone-600 px-0.5">/</span>
          <span className="text-amber-400 font-bold">{p.assists}</span>
        </span>
      </div>
    </div>
  );
