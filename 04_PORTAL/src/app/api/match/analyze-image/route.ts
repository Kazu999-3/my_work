...
【重要：チーム(サイド)の判定について】
LoLのリザルト画面は「上の5行＝勝利(VICTORY)チーム、下の5行＝敗北チーム」の順に並びます。**勝利チームが青サイド(BLUE)とは限りません。**
各ブロックがBLUE/REDどちらのサイドかは、行の配色から判定してください：
- サモナー名・数値・バー・左端マーカーなどが**青系**の色調 ➔ そのブロックは "BLUE"
- **赤系**の色調 ➔ そのブロックは "RED"
つまり「上のブロックの色」が赤なら、勝利チームはREDです。位置（上下）でBLUE/REDを決めることは絶対にしないでください。
"winningTeam" は「上の5行（勝利側）ブロックのサイド色」で決定します（上が青系ならBLUE、赤系ならRED）。
