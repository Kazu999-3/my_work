...
async function analyzeXPostImagesWithGemini(photos: any[], videos: any[], tweetText: string): Promise<string> {
  ...
  // 2. 添付動画メディアの収集 (MP4動画ファイルまたは高解像度サムネイル)
  if (videos && videos.length > 0) {
    videos.forEach((v: any, i: number) => {
      // MP4動画の直リンクがある場合は動画優先、無ければサムネイル
...
※要約して短くすることは固く禁止します。貴重な情報を一つも漏らさず全て網羅してください。`;
      ...
      console.warn(`Gemini Vision/Video analysis error for ${item.label}:`, err.message);
    }
  }
...
// ============================================================
// Gemini API で超詳細全情報網羅の解析・分類・タグ付けを実行（省略厳禁）
// ============================================================
...
1. 【全情報完全網羅ナレッジ作成】:
   - 動画内プレイ展開および画像内のビジュアル解析データ（全アイテム、全ルーン、スキル順、KDA、画面テキスト等）をそのまま書き残すこと。
   - 戦術・ビルド・立ち回りのロジック・ポイントを徹底的に解説すること。
