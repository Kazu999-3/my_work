...
// ★ 2026-09-22: recharts(約500KB)を直接importしていたため、このページの初回JSは
// 実測1,325KBと他ページ(684〜820KB)の倍近くあった。チャートを別モジュールへ切り出し、
// next/dynamic で遅延読込することで初回表示に必要なJSから recharts を外す。
const chartLoading = () => (
  <div className="w-full h-full min-h-[120px] rounded-xl bg-black/[0.03] animate-pulse" />
);
const HextechRadarChart = dynamic(() => import('./charts/PlayerCharts').then(m => m.HextechRadarChart), { ssr: false, loading: chartLoading });
const MiniMmrChart = dynamic(() => import('./charts/PlayerCharts').then(m => m.MiniMmrChart), { ssr: false, loading: chartLoading });
const MainMmrChart = dynamic(() => import('./charts/PlayerCharts').then(m => m.MainMmrChart), { ssr: false, loading: chartLoading });
const WinRateChart = dynamic(() => import('./charts/PlayerCharts').then(m => m.WinRateChart), { ssr: false, loading: chartLoading });
                  ...
                  <div className="bg-white/60 backdrop-blur-xl border border-black/10 rounded-3xl p-6 shadow-xl lg:col-span-2 space-y-6">
                    ...
                    {activeLane === 'ALL' ? (
                      ...
                      <div className="w-full h-72">
                        <MainMmrChart mmrChartData={mmrChartData} lane={activeLane} activeLane={activeLane} visibleTierBounds={visibleTierBounds} currentLaneMmr={currentLaneMmr} />
