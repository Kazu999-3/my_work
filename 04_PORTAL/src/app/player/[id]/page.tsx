            ...
            >
              ...
              {activeTab === 'summary' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  {/* ★ Junglepedia プレイスタイル詳細 & スライダー */}
                  <div className="bg-white/[0.02] backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-xl col-span-1 lg:col-span-3 space-y-6 mt-2">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/5 pb-4">
                      <div>
                        <h3 className="text-lg font-black flex items-center gap-2 text-white">
                          <Sparkles className="w-5 h-5 text-amber-400" />
                          <span>プレイスタイル詳細 (Junglepedia 準拠)</span>
                        </h3>
                        <p className="text-xs text-gray-400 mt-1">
                          各種ゲームログ（Kills/Deaths/Assists/CS/Vision/DPM）からプレイスタイルを自動抽出。
                        </p>
                      </div>
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="h-3 w-full bg-gray-900 rounded-full overflow-hidden border border-white/5 p-[1px] relative">
                                  <div 
                                    ...
                                    style={{ width: `${currentStyle.sliders.aggressive}%` }}
                                  ></div>
                                </div>
                              </div>
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="h-3 w-full bg-gray-900 rounded-full overflow-hidden border border-white/5 p-[1px]">
                                  <div 
                                    ...
                                    style={{ width: `${currentStyle.sliders.farming}%` }}
                                  ></div>
                                </div>
                              </div>
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="h-3 w-full bg-gray-900 rounded-full overflow-hidden border border-white/5 p-[1px]">
                                  <div 
                                    ...
                                    style={{ width: `${currentStyle.sliders.supportive}%` }}
                                  ></div>
                                </div>
                              </div>
                            </div>
                          ...
                          return (
                            <div className="flex flex-col gap-3">
                              {currentStyle.tags.map((tag: any) => (
                                <div 
                                  key={tag.id}
                                  className="group relative bg-white/[0.01] hover:bg-white/[0.04] border border-white/5 hover:border-cyan-500/20 p-3 rounded-2xl transition duration-200 cursor-help"
                                >
                    ...
                    {/* ★ 9分時点の対面スタッツ差分 (9-Min Matchup Diffs) */}
                    {(() => {
                      const currentStyle = playstyle?.[playstyleSource] || {
                        diffs: { goldDiff: 0, xpDiff: 0, csDiff: 0 }
                      };
                      const diffs = currentStyle.diffs || { goldDiff: 0, xpDiff: 0, csDiff: 0 };
                      
                      return (
                        <div className="space-y-4 pt-6 border-t border-white/5">
                          <h4 className="text-xs font-black text-gray-400 uppercase tracking-wider">9分時点の対面スタッツ差分 (9-Min Diff Charts)</h4>
                          
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            {/* 1. ゴールド差 */}
                            <div className="bg-black/35 p-4 rounded-2xl border border-white/5 space-y-3">
                              <div className="flex justify-between text-xs font-bold">
                                <span className="text-gray-400">ゴールド差 (Gold Diff)</span>
                                <span className={`font-mono font-black ${diffs.goldDiff >= 0 ? 'text-amber-400' : 'text-rose-500'}`}>
                                  {diffs.goldDiff >= 0 ? `+${diffs.goldDiff}` : diffs.goldDiff} G
                                </span>
                              </div>
                              <div className="relative w-full h-2.5 bg-gray-900 rounded-full overflow-hidden border border-white/5">
                                <div 
                                  className={`absolute h-full rounded-full transition-all duration-500 ${diffs.goldDiff >= 0 ? 'bg-amber-500 left-1/2' : 'bg-rose-500 right-1/2'}`}
                                  style={{ 
                                    width: `${Math.min(50, (Math.abs(diffs.goldDiff) / 1000) * 50)}%`,
                                    left: diffs.goldDiff >= 0 ? '50%' : 'auto',
                                    right: diffs.goldDiff < 0 ? '50%' : 'auto'
                                  }}
                                ></div>
                                <div className="absolute left-1/2 top-0 w-[1px] h-full bg-white/30"></div>
                              </div>
                              <div className="text-[10px] text-gray-500 text-center">
                                {diffs.goldDiff >= 0 ? '対面よりファーム/キル先行' : '対面へのゴールド先行を許容'}
                              </div>
                            </div>
                            ...
                            <div className="bg-black/35 p-4 rounded-2xl border border-white/5 space-y-3">
                              <div className="flex justify-between text-xs font-bold">
                                <span className="text-gray-400">経験値差 (XP Diff)</span>
                                <span className={`font-mono font-black ${diffs.xpDiff >= 0 ? 'text-indigo-400' : 'text-rose-500'}`}>
                                  {diffs.xpDiff >= 0 ? `+${diffs.xpDiff}` : diffs.xpDiff} XP
                                </span>
                              </div>
                              <div className="relative w-full h-2.5 bg-gray-900 rounded-full overflow-hidden border border-white/5">
                                <div 
                                  className={`absolute h-full rounded-full transition-all duration-500 ${diffs.xpDiff >= 0 ? 'bg-indigo-500 left-1/2' : 'bg-rose-500 right-1/2'}`}
                                  style={{ 
                                    width: `${Math.min(50, (Math.abs(diffs.xpDiff) / 1000) * 50)}%`,
                                    left: diffs.xpDiff >= 0 ? '50%' : 'auto',
                                    right: diffs.xpDiff < 0 ? '50%' : 'auto'
                                  }}
                                ></div>
                                <div className="absolute left-1/2 top-0 w-[1px] h-full bg-white/30"></div>
                              </div>
                              <div className="text-[10px] text-gray-500 text-center">
                                {diffs.xpDiff >= 0 ? '対面より経験値・Lv先行' : 'レーン引受/ロームによるロストあり'}
                              </div>
                            </div>
                            ...
                            <div className="bg-black/35 p-4 rounded-2xl border border-white/5 space-y-3">
                              <div className="flex justify-between text-xs font-bold">
                                <span className="text-gray-400">CS差 (CS Diff)</span>
                                <span className={`font-mono font-black ${diffs.csDiff >= 0 ? 'text-emerald-400' : 'text-rose-500'}`}>
                                  {diffs.csDiff >= 0 ? `+${diffs.csDiff}` : diffs.csDiff} CS
                                </span>
                              </div>
                              <div className="relative w-full h-2.5 bg-gray-900 rounded-full overflow-hidden border border-white/5">
                                <div 
                                  className={`absolute h-full rounded-full transition-all duration-500 ${diffs.csDiff >= 0 ? 'bg-emerald-500 left-1/2' : 'bg-rose-500 right-1/2'}`}
                                  style={{ 
                                    width: `${Math.min(50, (Math.abs(diffs.csDiff) / 15) * 50)}%`,
                                    left: diffs.csDiff >= 0 ? '50%' : 'auto',
                                    right: diffs.csDiff < 0 ? '50%' : 'auto'
                                  }}
                                ></div>
                                <div className="absolute left-1/2 top-0 w-[1px] h-full bg-white/30"></div>
                              </div>
                              <div className="text-[10px] text-gray-500 text-center">
                                {diffs.csDiff >= 0 ? '対面よりラストヒット精度高' : '対面からのプッシュ圧あり'}
                              </div>
                            </div>
                          </div>
                        </div>
