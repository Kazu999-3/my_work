...
export const systemDesignMarkdown = `# Sovereign OS & KTM Bot 詳細設計仕様書 (Detailed System Design Specification)
...
* **チーム戦力均衡化処理**:
  ロール配置確定後、以下の項目を評価値として算出し、BlueチームとRedチ�## 10. Webポータル＆KTM運用全機能詳細設計マトリクス (Full Functional Specification)
...
| **🌳 ジャングル周回統計ライブラリ** | 管理者 🔑 | \`/admin/clears\` | (フロントエンド静的データ) | - | 主要チャンプの周回タイム目標、カイトのコツを整理。インライン YouTube プレイヤーで再生可能。 |
...
### 10-2. 各主要機能の詳細解説とデータ流入仕様 (Detailed Functional Logic & Data Source)

各機能が「**どこからデータを取得しているか（データソース）**」「**どのようにデータが追加・更新されるか（自動/手動）**」の設計仕様です。

#### A. ⚖️ MMRバランサー・チーム分け機能
*   **データソース（取得元）**: Supabase \`ktm_players\` テーブル。登録されているプレイヤー名、各ロールのMMR（初期値: 1200）、不満度（Pity）を取得。
*   **データ追加・更新トリガー（自動/手動）**:
    *   **プレイヤーデータ**: 新規プレイヤーが Discord Bot 経由で参加、またはポータル管理者画面から手動追加された際にレコードが生成されます（自動追加はされません）。
    *   **MMR & Pity**: 後述の **「対戦結果登録」** が手動で実行された際に、対戦スコアに基づいて自動計算・更新（UPDATE）されます。
*   **計算方式 (ロール配置ペナルティの最小化)**:
    *   全プレイヤーのレーン配置パターンに対してペナルティを計算し、合計値が最も小さい組み合わせを算出します。
    *   **希望外レーン配置**: \`500,000 pt\` / **サブレーン配置**: \`20,000 pt\` / **NGレーン配置**: \`2,000,000 pt\`（絶対回避用）。
    *   **こだわり度 (Weight) の調整**: 「絶対（weight=1）」に設定されたレーンが外れた場合はペナルティを **50倍** に引き上げ、「柔軟（weight=3）」の場合は **1/4** に軽減。
...
*   **データソース（取得元）**: バランサーで確定したメンバー構成と、ユーザーがフォームに入力した「勝敗」「ゲームスコア」。
*   **データ追加・更新トリガー（自動/手動）**:
    *   **手動トリガー**: 管理者またはプレイヤーが結果登録画面（\`/balancer/record\`）で「結果を送信」ボタンを手動で押すことで動作します（試合終了時に自動で追加されることはありません）。
    *   送信ボタン押下後、Supabase の \`ktm_matches\` に1つの対戦履歴行が **自動追加 (INSERT)** され、関係した10名のプレイヤーの \`ktm_players.mmr\` および \`pity\` が **自動更新 (UPDATE)** されます。
*   **計算方式 (Elo レーティング)**:
...
*   **データソース（取得元）**: Supabase \`ktm_players.metadata\` カラム（キャッシュ用）、および Riot API (Match-V5) のソロQ履歴。
*   **データ追加・更新トリガー（自動/手動）**:
    *   **KTMカスタムデータ**: 上述の「対戦結果登録」の手動実行時に、過去のKTMログ全体から自動で再計算・更新されます。
    *   **ソロキュー（SoloQ）データ**: プレイヤー詳細画面（\`/player/[id]\`）にある **「ソロQ戦績同期」ボタンをユーザーが手動で押したタイミング** で Riot API から最新10試合をフェッチし、プレイスタイルやタグが自動計算されて \`ktm_players.metadata.playstyle_cache.soloq\` に **自動キャッシュ保存 (UPDATE)** されます（Riot API のレート制限を避けるため、バックグラウンドでの定期自動同期は行われません）。
*   **計算方式 (プレイ傾向スライダー)**:
...
*   **データソース（取得元）**: Riot Match Timeline V5 API (ソロQ同期時)、または Supabase \`ktm_matches\` (KTMカスタム戦績)。
*   **データ追加・更新トリガー（自動/手動）**:
    *   「ソロQ戦績同期」または「対戦結果登録」が実行された際、バックグラウンドで **自動的に計算・更新** されます。
*   **計算方式 (タイムライン逆算と推定)**:
...
#### E. 🌳 ジャングル周回統計ライブラリ
*   **データソース（取得元）**: フロントエンドのソースコード内（\`page.tsx\`）に定義された **静的マスターデータ定数**。
*   **データ追加・更新トリガー（自動/手動）**:
    *   **完全な手動管理（コードベース）**: 外部データベース（Supabase等）は参照しておらず、動画のURLやクリアタイムは \`INITIAL_CLEARS\` 配列に静的に定義されています。そのため、新しいチャンピオンや動画を追加・修正する場合は、**\`src/app/admin/clears/page.tsx\` のコードを直接書き換える** 必要があります（自動で追加・更新されることはありません）。

#### F. 🔍 ソロQ対面リアルタイム偵察 ＆ 敵ジャングラー分析
*   **データソース（取得元）**: リアルタイムの Riot API (Spectator-V5 / Active Game) および対象プレイヤーの直近のソロQ対戦履歴。
*   **データ追加・更新トリガー（自動/手動）**:
    *   **手動トリガー（完全リアルタイム使い捨て）**: 管理者がサモナー名（Riot ID）を入力し、「偵察開始」ボタンを **手動で押したタイミングでのみ** リアルタイムに Riot API からデータを引き出し、その場でメモリ上だけで計算・表示します。
    *   データベース（Supabase）には分析結果や検索履歴などは一切保存されず、完全に「その場限り」の使い捨てデータです（自動巡回や定期自動保存はされません）。

#### G. 🧠 自動要約ナレッジベース ＆ YouTube自動解析キュー
*   **データソース（取得元）**: Supabase \`personal_knowledge\` (ナレッジDB)、および \`matchup_sentinel\` (チャンプ辞典)。ローカルファイル \`kirei_queue.json\` (タスクキュー)。
*   **データ追加・更新トリガー（自動/手動）**:
    *   **手動追加**: 管理者がURLやメモをフォームに入力し、「追加」ボタンを手動で押した際に登録されます。
    *   **YouTube自動解析（自動追加）**: 
        *   登録されたURLが YouTube 動画の場合、SREデーモン（Python）が15分おきに自動で動画をダウンロード・解析・Geminiによる文字起こし要約を実行し、**ナレッジ記事を自動生成・追加 (INSERT)** します。
...
🛡️ **Sovereign OS & KTM Bot Detailed System Design Specification - Documented by Antigravity**��する画面です。
1. **新規動画の追加**:
