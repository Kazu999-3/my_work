...
`
  ...
  "ui_design": {
    title: "🎨 UI/UX ＆ デザインシステム",
    filename: "06_ui_design.md",
    content: `# 🎨 ポータル UI/UX ＆ デザインシステム

Sovereignポータル（04_PORTAL）全体のビジュアル表現、未来的なSFネオン演出、操作インタラクション、およびレスポンシブ動作に関する技術設計。

---

## 1. カラーパレット (Sovereign Cyberpunk Theme)
...
## 2. グラスモルフィズム (Glassmorphism) & ホバーアクション
...
## 3. サイドバー ＆ ナビゲーション UI 仕様 (\`components/Sidebar.tsx\`)
...
* **エラーバナーの統一**: Riot/Geminiキー切れ等のエラー時は、背景を \`bg-red-900/20\`、境界線を \`border-red-800/50\` とした赤色の半透明警告バナーを描画し、バグ情報を明示。
`
  }
};
