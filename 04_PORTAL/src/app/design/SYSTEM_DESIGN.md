...
### 10-2. 各主要機能の詳細解説と計算ロジック (Detailed Functional Logic)

#### A. ⚖️ MMRバランサー・チーム分け機能
*   **概要**: 参加登録されたプレイヤーを、実力（MMR）が最も均等になり、かつプレイヤーのレーン希望や待機状況（不満度）が最も満たされるように2つのチーム（Blue/Red）に分割します。
*   **計算方式 (ロール配置ペナルティの最小化)**:
    *   全プレイヤーのレーン配置パターンに対してペナルティを計算し、合計値が最も小さい組み合わせを算出します。
    *   **希望外レーン配置**: `500,000 pt`
    *   **サブレーン配置**: `20,000 pt`
    *   **NGレーン配置**: `2,000,000 pt`（絶対回避用）
    *   **こだわり度 (Weight) の調整**: 「絶対（weight=1）」に設定されたレーンが外れた場合はペナルティを **50倍** に引き上げ、「柔軟（weight=3）」の場合は **1/4** に軽減します。
    *   **Pity (不満度カウンター)**: 希望レーンから漏れるたびに Pity 値が加算（サブ配置で `+2`、希望外で `+5`、観戦/待機で `+10`）され、次回以降のペナルティ計算時にこの値が差し引かれることで、優先的にメインレーンに割り当てられるように自動調整します。
*   **計算方式 (戦力の平準化)**:
    *   ロール配置決定後、チーム全体のMMR合計値 of 差、および各レーンごとの「対面MMR差の2乗和」を算出し、それが最小になるような組み合わせへシャッフルします。

#### B. 📋 対戦結果登録と MMR 計算ロジック
*   **概要**: 試合終了時の勝敗およびメンバー構成に基づき、プレイヤーのレーンMMRおよび総合MMRを自動更新します。
*   **計算方式 (Elo レーティング)**:
    *   MMR の増減値（\(\Delta MMR\)）は、以下の Elo 算式を適用します：
        \[R_{new} = R_{old} + K \cdot (S - E)\]
        *   \(K\): 変動重み係数。標準で `32` を適用。
        *   \(S\): 試合結果の実績値（勝利 = `1`, 敗北 = `0`）。
        *   \(E\): 期待勝率。対面相手（同じレーンの敵）のMMRとの差に基づき以下で計算：
            \[E = \frac{1}{1 + 10^{(MMR_{opp} - MMR_{player}) / 400}}\]
    *   これにより、格上の対面に勝った場合は上昇幅が大きくなり、格下に負けた場合は減少幅が大きくなるように動的に調整されます。

#### C. 👤 プレイヤー詳細プロフィール ＆ プレイ傾向計算エンジン
*   **概要**: プレイヤーの過去戦績から、Aggressive（攻撃的）、Farming（成長優先）、Supportive（献身的）の3つのプレイ傾向スライダー値と、プレイスタイルタグを判定・キャッシュします。
*   **計算方式 (プレイ傾向スライダー)**:
    *   **Aggressive**: キル関与率（KP）、キル数、デス数、DPM（1分あたりダメージ量）、FB（ファーストブラッド）関与率などのスタッツをプールし、正規化して 0〜100% のスコアを算出。
    *   **Farming**: CS/min（1分あたりCS数）、ジャングルクリープ処理割合、ゴールド獲得シェアから正規化算出。
    *   **Supportive**: 視界スコア（Ward/min）、味方防御用シールド/回復量、アシストシェアから正規化算出。
*   **計算方式 (プレイタグ自動判定)**:
    *   `early-brawler` (序盤の戦闘狂): Aggressive > 75% かつ 15分以内のキル関与率が平均より有意に高い場合に付与。
    *   `speed-demon` (快速ファーマー): Farming > 75% かつ 15分時点の平均CS数が上位10%以内の場合に付与。
    *   `lane-bully` (レーンの支配者): 対面スタッツ差分（9分時点のゴールド/CS差）の平均値が一定のしきい値を超えて有利な場合に付与。

#### D. 9分時点の対面スタッツ差分（Diff Charts）計算
*   **概要**: 試合開始9分（ゲームの序盤レーン戦終了の目安）時点での対面とのゴールド差、XP差、CS差をグラフで可視化します。
*   **計算方式 (タイムライン逆算と推定)**:
    *   Riot Match Timeline V5 API からフレーム9（9分時点）のスタッツを直接取得します。
    *   タイムラインデータが存在しない過去ログ（またはRiot APIキー未稼働時）は、プレイヤーの最終スタッツ（最終ゴールド、最終CS）と、そのチャンピオンの平均的な「周回・レーン戦減衰カーブ係数（チャンピオン固有値）」を用いて、9分時点のスタッツを数学的に推定・補間します。

#### E. 🔍 ソロQ対面リアルタイム偵察 ＆ 敵ジャングラー分析
*   **概要**: Riot ID から現在進行中のゲームを検知し、敵のジャングラーの開始バフ、最初のGank（レーン関与）位置、および戦術対策をリアルタイム分析します。
*   **計算方式 (敵ジャングラー特定 & 初動予測)**:
    *   **ジャングラー特定**: 進行中ゲーム (Spectator-V5) の敵チーム全員のサモナースペルをスキャンし、`SummonerSmite` (ID: 11) を装備しているプレイヤーを特定。
    *   **開始バフ＆初動Gank予測**: 敵チャンピオンのクリア時間・ルート傾向（例: Nidaleeは青バフ ➜ 蛙 ➜ 狼のLv3速攻、Gravesは赤バフ ➜ 鳥 ➜ 狼のフルクリア）と、その敵プレイヤーの過去プレイスタイル（戦闘狂タグの有無など）を複合。
    *   「序盤にインベイドまたはLv3早期Gankを好む確率（戦闘意欲パラメータ）」と「フルクリアを優先する確率（ファームパラメータ）」をブレンドし、最も確率の高い開始バフ（赤/青）と初動関与ターゲットを文字列として自動判定します。
