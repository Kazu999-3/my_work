...
export default function PromptsAdmin() {
  ...
  return (
    <div className="min-h-screen bg-background text-stone-900 p-6 font-sans">
      ...
      <div className="max-w-7xl mx-auto mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-stone-500 text-sm mt-1">
            プロンプトルーティング（モデル選択・フォールバック・温度設定）を管理・調整します。
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        <AnimatePresence mode="wait">
          ...
          >
            {/* 左サイド: プロンプトID一覧 */}
            <div className="lg:col-span-4 bg-white/70 border border-black/10 rounded-xl p-4 backdrop-blur-md">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-sm font-semibold text-stone-500 uppercase tracking-wider flex items-center gap-2">
                  <Terminal size={14} className="text-cyan-700" /> プロンプト一覧
                </h2>
                <button
                  onClick={() => fetchPrompts()}
                  className="text-xs text-cyan-700 hover:text-cyan-800 flex items-center gap-1 transition-colors"
                >
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-3 rounded-lg border transition-all duration-200 active:scale-[0.99] flex flex-col gap-1 ${
                          isSelected
                            ? 'bg-gradient-to-r from-cyan-100 to-indigo-100 border-cyan-400 shadow-md shadow-cyan-900/10'
                            : 'bg-white border-black/10 hover:border-black/20'
                        }`}
                      >
                        <div className="flex items-center justify-between w-full">
                          <span className={`font-mono text-sm font-bold ${isSelected ? 'text-cyan-700' : 'text-stone-800'}`}>
                            {p.prompt_id}
                          </span>
                          <span className="text-[10px] bg-black/5 text-stone-500 px-2 py-0.5 rounded-full font-mono">
            ...
            {/* 右サイド: エディタフォーム */}
            <div className="lg:col-span-8 bg-white/70 border border-black/10 rounded-xl p-6 backdrop-blur-md relative">
              <AnimatePresence mode="wait">
                {!selectedPrompt ? (
                  <div className="flex flex-col items-center justify-center py-32 text-stone-500">
                    <Cpu size={48} className="text-stone-300 animate-pulse mb-3" />
                  ...
                  >
                    {/* 選択中の情報ヘッダー */}
                    <div className="border-b border-black/10 pb-4 mb-4 flex flex-col md:flex-row md:items-center justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          <Edit3 size={18} className="text-cyan-700" />
                          <span className="text-xl font-mono font-bold text-stone-800">{selectedPrompt.prompt_id}</span>
                        </div>
                        <p className="text-xs text-stone-500 mt-1">最終更新: {new Date(selectedPrompt.updated_at).toLocaleString()}</p>
                      </div>

                      {saveStatus.type && (
                        <div className={`text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 ${
                          saveStatus.type === 'success' ? 'bg-emerald-100 border border-emerald-200 text-emerald-700' : 'bg-rose-100 border border-rose-200 text-rose-700'
                        }`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="md:col-span-2">
                        <label className="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">説明</label>
                        <input
                          type="text"
                          value={description}
                          onChange={(e) => setDescription(e.target.value)}
                          placeholder="プロンプトの用途を入力"
                          className="w-full bg-white border border-black/10 rounded-lg px-3 py-2 text-sm text-stone-800 focus:outline-none focus:border-cyan-600 transition-colors"
                        />
                      </div>

                      <div>
                        <label className="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">メインモデル (Default Model)</label>
                        <select
                          value={defaultModel}
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">フォールバック先 (Fallback Model)</label>
                        <select
                          value={fallbackModel}
                      ...
                      <div className="md:col-span-2">
                        <div className="flex justify-between items-center mb-2">
                          <label className="text-xs font-semibold text-stone-500 uppercase tracking-wider">創造性度合い (Temperature): {temperature}</label>
                          <span className="text-[10px] text-stone-500">低いほど安定的・堅実、高いほど創造的</span>
                        </div>
                        <input
                          type="range"
                          min="0.0"
                          ...
                          className="w-full h-1 bg-black/10 rounded-lg appearance-none cursor-pointer accent-cyan-600"
                        />
                      </div>
                    </div>
                    ...
                    {/* システムプロンプト */}
                    <div>
                      <label className="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">システム指示 / ペルソナ (System Prompt)</label>
                      <textarea
                        rows={4}
                        value={systemPrompt}
                        onChange={(e) => setSystemPrompt(e.target.value)}
                        placeholder="AIエージェントの性格、背景知識、制約条件などを指定"
                        className="w-full bg-white border border-black/10 rounded-lg p-3 text-sm text-stone-800 font-mono focus:outline-none focus:border-cyan-600 transition-colors leading-relaxed"
                      />
                    </div>
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        <label className="text-xs font-semibold text-stone-500 uppercase tracking-wider">本文テンプレート (User Prompt Template)</label>
                        <span className="text-[10px] text-stone-500">※埋め込み変数 (例: {'{title}'}, {'{transcript}'}) は変更しないでください</span>
                      </div>
                      <textarea
                        rows={12}
                    ...
                    {/* 保存アクションエリア */}
                    <div className="border-t border-black/10 pt-4 flex justify-end gap-3">
                      <button
                        onClick={() => handleSelectPrompt(selectedPrompt)}
                        disabled={isSaving}
                        className="px-4 py-2 border border-black/10 rounded-lg text-sm text-stone-500 hover:text-stone-800 hover:bg-black/5 transition-colors active:scale-95 disabled:opacity-50"
                      >
                        リセット
                      </button>
                      <button
