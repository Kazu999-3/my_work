...
def gemini_summarize(title, channel, transcript):
    prompt = f"""あなたはLoL(League of Legends)戦略・プロ解説の超一流アナリストAIです。
以下のYouTube動画の字幕から、一切の省略なく、高密度で圧倒的な情報量の日本語攻略バイブル(Markdown)を作成してください。
...
動画内で語られているすべてのプレイ展開、ビルド、ルーン、アイテム選択の理由、スキル順、パワースパイク、立ち回りのロジック、状況判断、マッチアップ対策、およびプロ/高レートの思考プロセスを【一つも漏らさず全て網羅】してください。

【構成指針】:
1. 【動画概要＆本質テーマ】: この動画が伝えている核心的な戦術コンセプト。
2. 【ビルド・ルーン・アイテム選択の根拠】: 各アイテムやルーンを選ぶ理由、対面や構成に応じた派生。
3. 【序盤・中盤・終盤の立ち回り＆判断基準】: 時間帯ごとの具体的なアクション、ジャングルルート、ウェーブ管理、オブジェクト判断。
4. 【対面・マッチアップ＆集団戦のコツ】: 有利/不利なマッチアップ、ガンクの合わせ方、集団戦のポジショニング。
5. 【プロ/チャレンジャーの思考プロセス書き起こし】: 動画内で言及されている細かなコツ、注意点、失敗しがちなポイント。

【出力言語の絶対条件】: 字幕が英語でも出力は必ず日本語にすること。チャンピオン名・アイテム名・ルーン名などの固有名詞のみ英語表記（または一般的な日本語併記）にすること。

必ず以下のJSONフォーマットのみを出力してください（コードブロック禁止）:
{{"title":"<具体的で分かりやすい日本語記事タイトル>","summary":"<全情報を一つも漏らさず完全網羅した高密度・圧倒的ボリュームのMarkdownコンテンツ>","genre":"<LoL攻略/ビルド/マクロ/その他 から1つ>","tags":["<関連タグ最大5つ>"],"champion":"<主題のチャンピオン英語ID。無ければUnknown>"}}
