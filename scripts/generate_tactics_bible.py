...
CORE_CHAMPIONS_DATA = {
    "JarvanIV": {
        ...
        "summary": "EQノックアップとR（天崩地裂）による絶対的エンゲージと序盤ガンク特化。Lv2~3のガンクテンポでゲームを掌握し、中盤以降はサンダード・スカイの耐久バーストで集団戦を制圧する。",
        "power_spikes": "Lv2 (赤バフ直後のEQ解放時), Lv3 (3キャンプ速攻ガンク), Lv6 (R習得時), 1stコア (サンダード・スカイ完成時)",
        "lethal_threshold": "Lv6時: EQノックアップ ➔ AA (パッシブ割合DMG) ➔ Wスロウ ➔ Rで約750〜820 DMG (相手HP 65%圏内)",
        "phase1": "赤バフ ➔ Lv2即ガンク、または3キャンプ即ガンク。EQを先撃ちせず徒歩接近でフラッシュを切らせてからEQ後出し。",
        "phase2": "敵JGの位置を予測しつつカウンターガンク。ファージまたはウォーハンマーからサンダード・スカイを最速完成。",
        "phase3": "オブジェクト前で側面・裏から回り込み、敵主要キャリー（ADC/MID）をRの檻に閉じ込めて即座に分断・排除。",
        "trap_items": "初手プロフェイン・ハイドラ等のフル脅威特化ビルド（集団戦R突撃時に敵フォーカスを浴びて蒸発する。サンダード・スカイが確定解）",
        "forbidden_moves": "不用意な遠距離からの開幕EQぶっぱなし（外した瞬間にブリンク手段がゼロになり即死する）",
        "matchup_tip": "Graves 対面: 高ARと引き撃ちでタイマンは勝てないため1v1を避けガンクテンポ勝負。Poppy 対面: ポッピーのWが切れるまでEQを撃たない。"
    },
    "Lillia": {
    ...
    "Graves": {
        ...
        "summary": "E（クイックドロー）による高ARと引き撃ちカイト、煙幕（W）による視界遮断。序盤から森を支配するパワーファーム型キャリーJG。",
        "power_spikes": "Lv3 (Eスタック維持時), 1stコア (コレクター / オポチュニティ完成時), 2ndコア (ロード・ドミニク / インフィニティ・エッジ)",
        "lethal_threshold": "Lv6時: E ➔ AA ➔ Q壁反射 ➔ W ➔ Rで約780〜850 DMG",
        "phase1": "Eスタックを絶やさずキャンプを高速周回。敵JGの逆サイド森へ積極的に侵入（カウンターJG）してキャンプを奪う。",
        "phase2": "煙幕で敵の視界を奪いながらオブジェクトをソロで狩る。1v1の殴り合いはEのAR上昇でほぼ全勝可能。",
        "phase3": "敵前衛を引き撃ち（カイト）で溶かしつつ、R（コラテラル・ダメージ）で後衛の瀕死キャリーを広範囲爆撃。",
        "trap_items": "防御ステータスを一切積まない無謀なレイトビルド（APバースト一撃で溶けるため、相手APに応じてマルモティウスやエッジ必須）",
        "forbidden_moves": "Qを障害物のない開けた場所で雑に撃つこと（壁反射させないと2段目爆破が当たらずDPS半減）",
        "matchup_tip": "JarvanIV 対面: J4のEQをEダッシュで横に躱せば一方的に蜂の巣にできる。カモ対面。"
    },
    "Kindred": {
        "title": "キンドレッド (Kindred) 対面戦術バイブル",
        "tags": ["LoL", "Tactics", "Jungle", "Kindred"],
        "summary": "パッシブマークによる射程・攻撃力無限スケーリングと、R（羊の果てぬ救済）による集団戦リセット型マークスマンJG。",
        "power_spikes": "マーク4スタック達成時 (射程+75), 1stコア (クラーケンスレイヤー完成時), Lv6 (R習得時)",
        "lethal_threshold": "Lv6時: Qカイト ➔ W領域展開 ➔ E3段目クリティカル処刑で約700 DMG",
        "phase1": "マーク出現位置を先読み。味方レーン主導権（Prio）がある側にのみ寄り、無理なインベードは避ける。",
        "phase2": "壁越えQを活用した高速カウンターガンク。ドラゴン・ヘラルド戦でRを展開し、敵のバーストを無効化。",
        "phase3": "マーク7スタック以上の超長射程で敵前衛をアウトレンジ。Rの無敵終了直前にE3段目を叩き込み確実にキル。",
        "trap_items": "マーク集めに固執した無謀な単独侵入（敵に囲まれてデスしゲームが崩壊する最大の罠）",
        "forbidden_moves": "敵に回復阻害や処刑スキルが残っている状態での不用意なR発動（敵を延命させてしまう事故）",
        "matchup_tip": "Master Yi 対面: イーのQを耐えてRを展開。R終了瞬間にE3段目で即座に処刑する。"
    },
    "Amumu": {
        "title": "アムム (Amumu) 対面戦術バイブル",
        "tags": ["LoL", "Tactics", "Jungle", "Amumu"],
        "summary": "Qの2連包帯投げスタンと、R（めそめそミイラの呪い）による超広範囲行動不能。集団戦イニシエート最強格タンクJG。",
        "power_spikes": "Lv3 (Q2回チャージ解放), Lv6 (R呪い習得時), 1stコア (サンファイア・イージス完成時)",
        "lethal_threshold": "Lv6時: Qスタン ➔ W+E連打 ➔ R全体拘束で味方追撃確殺ライン",
        "phase1": "WとEによる高速フルクリア。無理な1v1は避け、レーンが引いている味方へQスタンガンク。",
        "phase2": "ドラゴン・バロン前で敵集団を索敵。フラッシュRで敵チーム全員を足止めし、集団戦を完全掌握。",
        "phase3": "敵キャリーにQで張り付き、EのCD短縮（被弾によるリセット）を活かして範囲魔法ダメージを撒き散らす。",
        "trap_items": "耐久を捨てたフルAP特化（集団戦で飛び込んだ瞬間にスタンを入れる前に蒸発する）",
        "forbidden_moves": "味方の追撃が届かない距離で単独Q→Rダイブすること",
        "matchup_tip": "Kha'Zix 対面: 孤立ダメージを受けると即死するため、ブッシュチェックは慎重に行い味方の近くで戦う。"
    },
    "Khazix": {
        "title": "カ＝ジックス (Kha'Zix) 対面戦術バイブル",
        "tags": ["LoL", "Tactics", "Jungle", "Khazix"],
        "summary": "パッシブ孤立判定による超絶バーストダメージと、キルリセット跳躍（E進化）による連続暗殺アサシンJG。",
        "power_spikes": "Lv6 (Q進化), 1stコア (プロフェインハイドラ / ゴーストブレード完成時), Lv11 (EまたはW進化)",
        "lethal_threshold": "Lv6孤立時: 飛びつき ➔ Q (孤立倍率) ➔ AA ➔ Wで相手HP 80%が消滅",
        "phase1": "孤立した敵JGへの積極的インベード。川でのスカトル争奪やタイマンでファーストブラッドを狙う。",
        "phase2": "敵のサイドプッシュをブッシュで待ち伏せ。孤立した敵を1秒で暗殺し、即座にRステルスで離脱。",
        "phase3": "集団戦では先陣を切らず、敵の主要スキルが落ちた瞬間、孤立した敵バックラインへ飛び込みキルリセットで連鎖暗殺。",
        "trap_items": "孤立が取れない敵構成（タンク偏重・密集構成）に対して無理に脅威を重ね積みすること",
        "forbidden_moves": "集団戦の開幕で敵5人の真ん中に真っ先に飛び込むこと（即座にフォーカスされて蒸発する）",
        "matchup_tip": "Rengar 対面: レベル6以降、先に姿を見せた方が負ける心理戦。オラクルレンズでレンガーのステルスを看破する。"
    }
}
