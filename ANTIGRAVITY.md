...
> ⚠️ **2026-07-26 削除済みの収益化パイプラインについて**: 本書に記載の収益化パイプライン（`_MONETIZE/`、`agents/`、`monetization_batch.py`、`tool_scout.py`／`tool_forge.py`、note-publisher等）は、設計を見直した上で改めて実装するため2026-07-26に一旦削除済みです。以下の記述は実装記録として過去の到達点を残しているものであり、現在稼働しているものではありません。現存するのは `promoter.py`（`pulse.py` の定期実行から利用）のみです。詳細は `HANDOVER_CLAUDE.md` を参照してください。
...
5. **Sovereign Mind (ティルト防止メンタルチェッカー) [UPGRADE]**:
   - Python + Next.js ポータル上の試合終了後自動ポップアップ (`TiltDiagnosisPopup.tsx`)。
   - **Step 1 (1秒直感チェック)**: 試合直後に敗因の感じ方（味方/自分/構成/未定）を4択でクイック入力。
   - **Step 2 (AIテキスト感情・認知バイアス解析)**: 振り返りメモの文章トーン（「他罰的イライラ」vs「冷静な客観分析」）をGeminiで判別。
   - **統合スコアリング**: 1秒チェック(30%) + 戦績/連敗数(30%) + AIテキスト感情解析(40%)の重み付けで判定。他罰イライラ度70%以上でアンガーデトックス画面へ自動接続。
6. **Sovereign HUD (リアルタイムインゲームオーバーレイ) [NEW]**:
...
## 👑 The Sovereign Victory Loop (完全勝利サイクル憲法)

本システムは、**「プレイ前（ナレッジ） ➔ プレイ中（オーバーレイ） ➔ プレイ後（アナリティクス） ➔ 次のプレイ前へのナレッジ蓄積」** という、プレイヤーの実力・勝率を爆発的に引き上げる単一の完全自律サイクルとして運用されます。

### 🏛️ 2大開発原則（最上位誓約）
1. **完全自律 勝利ループの維持**:
   - ナレッジ（辞典/コーチ）、オーバーレイ（HUD）、アナリティクス（試合後レビュー）の3者が孤立せず、共通DB（Supabase/LiveClientData）を通じてシームレスに情報が循環すること。
2. **不確定情報の完全排除（ファクト100%主義）**:
   - **AIの推測やでっち上げ、裏付けのない数字は100%排除する。**
   - すべてのデータは以下の「確定した公式・実戦ソース」のみから計算・取得する：
     1. **Riot公式 Live Client Data API**（試合中の実データ：ゴールド、CS、キル、アイテム、レベル、スキルCD、イベントログ）
     2. **Riot公式 DataDragon API**（チャンピオンの正確な基礎ステータス、スキルの公式ダメージ計算式・スケーリング比率、アイテムステータス）
     3. **ユーザー本人の実戦データ（Supabase）**（過去の対戦履歴、確定した対面メモ、勝敗実績）
     - ※ *「計算式が不明なもの」「裏付けの取れない推測」は一切表示せず、確実にソースのある確定情報のみを厳密に提供する。*

### 🗺️ 開発ロードマップ (Step 1 〜 Step 4)
- **Step 1: 【プレイ前ナレッジ ＆ 対面計算基盤】**
  - 案A: DataDragon公式確定ダメージ計算による「即死キルライン境界メーター」
  - 案B: Supabase実戦対面メモの構造化「3段階勝ちパターン手順書」
- **Step 2: 【プレイ中オーバーレイ連動】**
  - Sovereign HUD (TAB画面・左側カード) への即死ライン・フェーズ手順・逆転コンパス（案C）のリアルタイム反映
- **Step 3: 【プレイ後ディープアナリティクス】**
  - 案1: 15分序盤メトリクス精密アナリティクス (CS推移・トレード比)
  - 案E: リコール＆ウェーブのテンポロス（もしもの世界）タイムライン逆再生
  - 案6: ビルド選択の分岐監査 (Build Audit: 重傷・靴購入タイミングの採点)
  - 案4: 時間帯別スケーリング勝率 ＆ パワースパイク分析
  - 案9: 目標ランク（ダイヤ/エメラルド）との多角形ギャップレーダー
- **Step 4: 【勝利ループの結合 ＆ ナレッジ自動更新】**
  - 試合後のファイト勝敗・改善ポイントがチャンピオン辞典・対面メモに自動蓄積され、次回のプレイ前ナレッジへ循環する完全サイクルの検証
...
> ⚠️ 下記フォルダ構成のうち `_MONETIZE/` および `agents/`、`monetization_batch.py` は2026-07-26に削除済みです（冒頭の注記を参照）。
...
- [x] **[フェーズ9]** YouTube自動チャンネル監視 ＆ 新着動画の自動解析インポート
  ...
  - [x] **[フェーズ11]** ソロQ 1分振り返り統合導線（SoloQ Reflection Flow）
  - [x] **[ステップ1]** Riot APIから直近のソロQ戦績（勝敗、使用/対面チャンプ、KDA、CS）を取得・入力補完するAPI `/api/soloq/latest` の実装
  - [x] **[ステップ2]** 振り返り記録テーブル `soloq_reflections` の DB スピーディ構築（メンタル度、勝因敗因タグ、対面メモ、次回テーマ）
  - [x] **[ステップ3]** コーチページ（`/coach`）トップに「⚡ 1分ソロQ振り返り」統合モーダルの設置
  - [x] **[ステップ4]** 入力された対面メモを既存 `matchup_sentinel`（対面DB）へ自動アペンド・同期するロジックの実装
  - [x] **[ステップ6]** 重複機能の削減・統合（MatchupMemoTabの廃止、PostGameTabの統合、試合前/試合後の2ステップUI化）
  - [x] **[ステップ7]** ソロQ振り返りUXの劇的改善（Riot IDのlocalStorage自動記憶、直近5試合一覧からの自由選択、勝敗連動ワンタップタグ＆入力フォーム最適化）
- [x] **[フェーズ12]** ソロQ完全勝利ナビゲーション ＆ 試合終了自動検出パイプライン
  - [x] **[ステップ1]** BAN/PICK中：推奨ピックナビゲーター（味方シナジー ＆ 敵カウンター Top3 推薦）の実装
  - [x] **[ステップ2]** 試合前：5レーン主導権（プッシュ優先権メーター）＆ 試合展開シミュレーターの実装
  - [x] **[ステップ3]** 試合前：対面確定時の「過去の自分からの警戒メモ」自動ハイライト表示機能の実装
  - [x] **[ステップ4]** 試合直後：ソロQ終了のバックグラウンド自動検知 ＆ 振り返りモーダル自動ポップアップ機能の実装
- [x] **[フェーズ14]** 攻略ナレッジ統合ハブ ＆ ノイズレス・ゼロ遷移UI再設計
- [x] **[フェーズ15]** 総合ガイド ＆ アップデート統合ハブ (`/guide`) の構築
  - [x] **[ステップ1]** 初心者向け3ステップ登録・参加ガイド (`GuideQuickStartTab`)
  - [x] **[ステップ2]** Discord Bot（募集・レーン設定・便利コマンド・ボタン操作）完全図解ガイド (`GuideBotTab`)
  - [x] **[ステップ3]** ポータル主要機能（マイページ・コイン＆ショップ・相性分析・バランサー）ガイド (`GuidePortalTab`)
  - [x] **[ステップ4]** アップデート情報（リリースノート）統合 ＆ 「📋 Discord告知文ワンタップコピー」機能 (`GuideUpdatesTab`)
...
> ⚠️ このセクションで説明する `tool_scout.py`・`tool_forge.py`・`NotePublisher` 等は2026-07-26に削除済みです（冒頭の注記を参照）。再実装時の設計参考として残しています。
...
> ⚠️ 以下「1. Monetization Batch」「2. note PVアクセス統計 ＆ 自己進化」は2026-07-26に削除済みです（冒頭の注記を参照）。「3. 相性・ライバル勝率マトリクス」（`/api/player/chemistry`）は現役で稼働中です。
...
### 5. 複数レーン（ロール別）チャンピオン辞典仕様 (Multi-Lane Champion Facts)
- **ロール別完全分離**:
  - ヤスオ、グラガス、ポッピーなど複数レーンで運用されるチャンピオンは、`champion_facts` の `(champion, role)` 単位でビルド・ルーン・基本立ち回り・パワースパイクを個別に保持します。
  - チャンピオン詳細モーダル上部にそのチャンピオンのプレイ可能ロール（`champion_lane_roles` 由来の TOP / MID / BOT 等）切替タブを表示し、ワンクリックでレーンごとの知見を切り替えて閲覧・編集・保存可能です。
- **フォールバック設計**:
  - 指定されたロールの知見が未作成の場合は、最新の共通/メインロール知見を安全にフォールバック表示します。
...
### 3. KTMポータル ページ・メニュー権限マッピング憲法（固定仕様）
KTMポータルのメニュー表示およびページアクセス権限は、以下の単一の真実（SSoT）に従って厳格に固定運用されます：

- **🌐 一般ユーザー向けメニュー（6大コア機能）**:
  1. `マイページ / 希望レーン` (`/mypage` / `/player/[id]`): アカウント設定、希望レーン、戦績カルテ、所持コイン
  2. `師弟自己紹介掲示板` (`/mentorship`): 師弟マッチング、自己紹介カード投稿・閲覧
  3. `チーム分けバランサー` (`/balancer`): チーム分け結果確認、ドラフト、BO3進行
  4. `勝敗予想 (カジノ)` (`/casino`): 試合勝敗ベット、アイテム購入
  5. `順位表 ＆ 名簿` (`/leaderboard`): レーティングランキング、名簿一覧、シナジー分析
  6. `使い方 ＆ 更新情報` (`/guide`): カスタム対戦ルール、ロール仕様、機能ガイド、更新ログ

- **🛡️ 管理者専用メニュー（管理者認証時のみ表示・アクセス）**:
  1. `チャンピオン攻略辞典` (`/champions`): パッチ別知見編集、ビルド・ルーン・対面メモ管理
  2. `パーソナルコーチ` (`/coach`): AIコーチング管理、ソロキュー反省機能
  3. `KTM大会管理` (`/ktm-admin`): 大会進行、メンバー登録、結果確定
  4. `システム運用` (`/admin/dashboard`): サーバー死活監視、DBヘルスチェック
  5. `note分析` (`/admin/analytics`): noteアクセス解析、記事パフォーマンス
