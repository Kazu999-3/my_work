...
- [ ] **[フェーズ8]** チャンピオン辞典のメタ自動アップデート ＆ プロビルド追跡統合
  - [ ] lol_trend_collector.py 等から得られる現パッチの勝率急上昇・強アイテムデータの自動抽出
  - [ ] pro-build-tracker (Oner/Canyon等のop.gg追跡) と連動したプロの最先端ビルドの自動抽出
  - [ ] dict_synthesizer.py を拡張し、パッチメタ情報とプロビルド情報を辞典（matchup_sentinel）へ自動でアペンド・マージするロジックの実装
  - [ ] ポータルUIでの「最新トレンド ＆ プロ推奨ルーン・ビルド」の視覚的表示対応
- [ ] **[フェーズ9]** YouTube自動チャンネル監視 ＆ 新着動画の自動解析インポート
  - [ ] 登録チャンネル管理テーブル `youtube_channels` の Supabase での設定
  - [ ] チャンネルURL（@ハンドル等）からチャンネルID/名前を解決して登録する機能の実装
  - [ ] ポータル上のチャンネル登録・監視リスト表示UIの開発
  - [ ] RSSフィードを用いたAPI無料での新着動画巡回検知＆自動キューイングバッチ `youtube_monitor.py` の実装
  - [ ] SREデーモンへの巡回タスク（3時間おき）の統合と自律実行の検証
